    ...
    public class OcrAccuracyService : IOcrAccuracyService
    {
        ...
        public async Task<OcrAccuracyScore> CalculateAccuracyAsync(string recognizedText, string groundTruth)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    _logger.LogWarning("Referans metin boş, doğruluk hesaplanamıyor");
                    return score;
                }
                ...
                _logger.LogInformation($"Doğruluk hesaplandı - Genel: {score.OverallScore:P}, Karakter: {score.CharacterAccuracy:P}, Kelime: {score.WordAccuracy:P}");
        ...
        public async Task<OcrAccuracyReport> GenerateDetailedReportAsync(List<OcrTestResult> testResults)
        {
            return await Task.Run(() =>
            {
                ...
                if (!testResults.Any())
                {
                    _logger.LogWarning("Rapor oluşturmak için test sonucu sağlanmadı");
                    return report;
                }
                ...
                _logger.LogInformation($"Detaylı rapor oluşturuldu: {report.TotalTests} test, genel doğruluk: {report.OverallAccuracy:P}");
        ...
        private Dictionary<string, object> GenerateRecommendations(OcrAccuracyReport report)
        {
            ...
            {
                recommendations["AccuracyImprovement"] = "Genel doğruluk %80'in altında. Görüntü ön işlemeyi iyileştirmeyi veya farklı OCR motorları denemeyi düşünün.";
            }
            foreach (var summary in report.EngineSummaries.Values)
            {
                if (summary.AverageAccuracy < 0.7)
                {
                    recommendations[$"{summary.EngineType}Improvement"] = $"{summary.EngineType} düşük doğruluğa sahip ({summary.AverageAccuracy:P}). Parametre ayarı yapmayı veya farklı ön işleme kullanmayı düşünün.";
                }
            }
