...
namespace GameTranslatorUltimate
{
    public class TesseractOcrEngine : IOcrEngine
    {
        ...
        public async Task<string> RecognizeTextAsync(Bitmap image, string language, PageSegMode psm = PageSegMode.Auto)
        {
            ...
            // Güven skoru aşırı düşükse halüsinasyonları önlemek için 
            float threshold = (_appSettings?.EnableHandwritingMode == true) ? 0.35f : 0.65f;
            if (finalConfidence < threshold)
            {
                _logger.LogWarning($"OCR metin buldu ({finalResult}) ancak güven skoru (%{finalConfidence * 100:F0}) çok düşük (Eşik: %{threshold * 100:F0}) olduğu için metin çöpe atıldı.");
                return string.Empty;
            }
            ...
            return await Task.Run(() =>
            {
                try
                {
                    using (var preprocessedPix = preprocessStrategy(image))
                    {
                        ...
                        using (engine)
                        {
                            // El yazısı modu açıksa kısıtlayıcı modlardan kaçın (SingleLine gibi)
                            var psmToUse = (_appSettings?.EnableHandwritingMode == true && psm == PageSegMode.SingleLine) 
                                            ? PageSegMode.Auto 
                                            : psm;

                            engine.DefaultPageSegMode = psmToUse;
        ...
        private Pix Preprocess_AdaptiveThreshold(Bitmap image)
        {
            try
            {
                // El yazısı modunda görüntü OcrService'de işlendi (binarize edildi), 
                // bu yüzden burada tekrar eşikleme yaparak detayları bozmuyoruz.
                if (_appSettings?.EnableHandwritingMode == true)
                {
                ...
                {
                    int blockSize = _appSettings?.AdaptiveThresholdBlockSize ?? 11;
                    if (blockSize % 2 == 0) blockSize++;
                    int C = _appSettings?.AdaptiveThresholdC ?? 2;
        ...
        private void ConfigureHandwritingMode(TesseractEngine engine)
        {
            try
            {
                ...
                engine.SetVariable("textord_min_xheight", "4"); // Daha küçük harfler için 5 -> 4
                engine.SetVariable("textord_force_make_prop_words", "F");

                // Sözlük tabanlı "tahminleri" kapatıyoruz çünkü el yazısını genellikle yanlış kelimelere benzetiyorlar
                ...
                // LSTM tabanlı modern Tesseract için iyileştirmeler
                engine.SetVariable("lstm_choice_mode", "2");

                _logger?.LogInformation("El yazısı modu Tesseract için optimize edildi (Sözlükler devre dışı)");
            }
            catch (Exception ex)
            {
