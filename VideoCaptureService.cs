    ...
    public class VideoCaptureService : IVideoCaptureService, IDisposable
    {
        ...
        public async Task<bool> StartCaptureAsync(int deviceIndex = 0)
        {
            try
            {
                if (IsCapturing)
                {
                    _logger.LogWarning("Video yakalama zaten Ã§alÄ±ÅŸÄ±yor");
                    return true;
                }
                ...
                {
                    _logger.LogError($"Video yakalama cihazÄ± {deviceIndex} aÃ§Ä±lamadÄ±");
                    OnVideoError($"Video yakalama cihazÄ± {deviceIndex} aÃ§Ä±lamadÄ±");
                    return false;
                }

                // Video yakalama ayarlarÄ±nÄ± yapÄ±landÄ±r
                ...
                _logger.LogInformation($"Video yakalama baÅŸlatÄ±ldÄ± - Cihaz: {deviceIndex}, Ã‡Ã¶zÃ¼nÃ¼rlÃ¼k: {actualWidth}x{actualHeight}, FPS: {actualFps}");
            ...
            {
                _logger.LogError("Video yakalama baÅŸlatÄ±lamadÄ±", ex);
                OnVideoError("Video yakalama baÅŸlatÄ±lamadÄ±", ex);
                return false;
            }
        }
        ...
        public async Task StopCaptureAsync()
        {
            ...
            {
                _logger.LogError("Video yakalama durdurulurken hata oluÅŸtu", ex);
                OnVideoError("Video yakalama durdurulurken hata oluÅŸtu", ex);
            }
        }
        ...
        public async Task<Bitmap> CaptureFrameAsync()
        {
            return await Task.Run(() =>
            {
                ...
                {
                    _logger.LogError("Tek bir kare yakalanÄ±rken hata oluÅŸtu", ex);
                }
        ...
        public string[] GetAvailableDevices()
        {
            ...
            {
                _logger.LogError("Mevcut video cihazlarÄ± algÄ±lanÄ±rken hata oluÅŸtu", ex);
            }
        ...
        private async Task CaptureFramesAsync(CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogError("Video yakalama dÃ¶ngÃ¼sÃ¼nde hata oluÅŸtu", ex);
                OnVideoError("Video yakalama dÃ¶ngÃ¼sÃ¼nde hata oluÅŸtu", ex);
            }
        }
