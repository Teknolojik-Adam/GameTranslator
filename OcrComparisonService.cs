...
namespace GameTranslatorUltimate
{
    public class OcrComparisonService : IOcrComparisonService, IDisposable
    {
        ...
        public async Task<OcrComparisonResult> CompareEnginesAsync(Bitmap image, string language)
        {
            ...
            try
            {
                _logger.LogInformation($"{language} dili iÃ§in OCR karÅŸÄ±laÅŸtÄ±rmasÄ± baÅŸlatÄ±lÄ±yor.");
            ...
            {
                _logger.LogError("OCR karÅŸÄ±laÅŸtÄ±rmasÄ± sÄ±rasÄ±nda hata oluÅŸtu", ex);
            }
            finally
            {
                stopwatch.Stop();
                result.TotalProcessingTime = stopwatch.Elapsed;
                _logger.LogInformation($"KarÅŸÄ±laÅŸtÄ±rma {result.TotalProcessingTime.TotalMilliseconds:F2}ms'de tamamlandÄ±. En iyi motor: {result.BestEngine}");
