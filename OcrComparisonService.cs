...
namespace P5S_ceviri
{
    public class OcrComparisonService : IOcrComparisonService, IDisposable
    {
        ...
        public async Task<OcrComparisonResult> CompareEnginesWithRegionsAsync(Bitmap image, string language)
        {
            ...
            try
            {
                _logger.LogInformation($"{language} dili için bölgelerle OCR karşılaştırması başlatılıyor.");
                var regions = DetectTextRegions(image);
                if (!regions.Any())
                {
                    _logger.LogWarning("Metin bölgesi tespit edilemedi, tam görüntü karşılaştırmasına geçiliyor.");
                    return await CompareEnginesAsync(image, language);
                }
                
                _logger.LogInformation($"{regions.Count} metin bölgesiyle paralel karşılaştırma başlatılıyor.");
            ...
            {
                _logger.LogError("Bölgelerle OCR karşılaştırması sırasında hata oluştu", ex);
            }
            finally
            {
                stopwatch.Stop();
                result.TotalProcessingTime = stopwatch.Elapsed;
                _logger.LogInformation($"Bölgeli karşılaştırma {result.TotalProcessingTime.TotalMilliseconds:F2}ms'de tamamlandı. En iyi motor: {result.BestEngine}");
                OnComparisonCompleted(new OcrComparisonCompletedEventArgs(result));
            }
            return result;
        }
        ...
        private async Task<OcrEngineResult> ProcessEngineAsync(OcrEngineType engineType, IOcrEngine engine, Bitmap image, string language)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.Confidence = 0;
                _logger.LogError($"{engineType} işlenirken hata oluştu", ex);
            }
            return result;
        }
        ...
        public async Task<OcrComparisonReport> GenerateComparisonReportAsync(List<OcrComparisonResult> results)
        {
            return await Task.Run(() =>
            {
                ...
                _logger.LogInformation($"Doğruluk raporu oluşturuldu: {report.TotalComparisons} karşılaştırma, en iyi motor: {report.OverallBestEngine}");
                return report;
            });
        }
        ...
        private void GenerateRecommendations(OcrComparisonReport report)
        {
            ...
            {
                recommendations["PerformansUyarisi"] = "İşlem süresi yüksek. Görüntü çözünürlüğünü düşürmeyi veya daha hızlı motorlar kullanmayı düşünün.";
            }

            if (report.EngineStats.Values.Any(s => s.SuccessRate < 0.7))
            {
                recommendations["DogrulukUyarisi"] = "Bazı motorların başarı oranları düşük. Görüntü ön işleme iyileştirmelerini düşünün.";
            }
        ...
        private List<Rectangle> DetectTextRegions(Bitmap image)
        {
            ...
            {
                _logger.LogError("Metin bölgeleri tespit edilirken hata oluştu", ex);
            }
            return regions;
        }
