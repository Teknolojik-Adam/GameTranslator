    ...
    public class OcrService : IOcrService, IDisposable
    {
        ...
        private Bitmap PreprocessImageForOcr(Bitmap originalImage, bool invertColors = false)
        {
            ...
            try
            {
                ...
                if (_appSettings.EnableHandwritingMode)
                {
                    processedImage = PreprocessForHandwriting(processedImage);
                }
        ...
        private Bitmap PreprocessForHandwriting(Bitmap image)
        {
            try
            {
                ...
                using (Mat gray = src.CvtColor(ColorConversionCodes.BGR2GRAY))
                {
                    // Gürültü giderme ve kontrast artırma
                    Mat denoised = new Mat();
                    Cv2.MedianBlur(gray, denoised, 3);

                    // Adaptif eşikleme kullanarak yazıyı ön plana çıkar
                    Mat binary = new Mat();
                    Cv2.AdaptiveThreshold(denoised, binary, 255, AdaptiveThresholdTypes.GaussianC, ThresholdTypes.Binary, 31, 10);

                    // Görüntüyü biraz kalınlaştır (erozyon - siyah yazı olduğu için)
                    Mat kernel = Cv2.GetStructuringElement(MorphShapes.Rect, new OpenCvSharp.Size(2, 2));
                    // Cv2.Erode(binary, binary, kernel); // Yazıyı biraz kalınlaştırır

                    var result = BitmapConverter.ToBitmap(binary);
                    denoised.Dispose();
                    binary.Dispose();
                    kernel.Dispose();
                    return result;
            ...
            {
                _logger.LogError("El yazısı ön işleme başarısız oldu, orijinal görüntüyle devam ediliyor.", ex);
                return image;
            }
        }
        ...
        private float DetectSkewAngle(Bitmap image)
        {
            ...
            try
            {
                ...
                {
                    // Determine dominant brightness to decide whether to invert or not
                    double mean = gray.Mean().Val0;
                    if (mean > 127) // Clear background, dark text
                    ...
                    // Dilate horizontally to merge characters into lines for better orientation detection
                    using (Mat kernel = Cv2.GetStructuringElement(MorphShapes.Rect, new OpenCvSharp.Size(30, 2)))
                    {
                    ...
                    foreach (var contour in contours)
                    {
                        ...
                        if (area < 150) continue; // Skip noise

                        var rect = Cv2.MinAreaRect(contour);
                        float angle = rect.Angle;

                        // Handle OpenCV version differences and orientation
                    ...
                    // Use median angle for robustness against outliers (UI elements)
                    angles.Sort();
                    float medianAngle = angles[angles.Count / 2];

                    if (Math.Abs(medianAngle) > 0.1f)
                        _logger?.LogInformation($"Skew detected: {medianAngle:F2} degrees");
            ...
            {
                _logger?.LogError("Error detecting skew angle", ex);
                return 0f;
            }
        }
        ...
        private Bitmap CorrectSkew(Bitmap image, float angle)
        {
            ...
            using (Mat src = BitmapConverter.ToMat(image))
            using (Mat rotationMatrix = Cv2.GetRotationMatrix2D(new Point2f(src.Width / 2.0f, src.Height / 2.0f), -angle, 1.0))
            {
                ...
                rotationMatrix.Set(0, 2, rotationMatrix.At<double>(0, 2) + (newWidth / 2.0) - (src.Width / 2.0f));
                rotationMatrix.Set(1, 2, rotationMatrix.At<double>(1, 2) + (newHeight / 2.0) - (src.Height / 2.0f));
