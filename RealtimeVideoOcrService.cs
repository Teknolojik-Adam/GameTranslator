    ...
    public class RealtimeVideoOcrService : IRealtimeVideoOcrService, IDisposable
    {
        ...
        public async Task<bool> StartAsync(int deviceIndex = 0)
        {
            try
            {
                ...
                // Video yakalamayÄ± baÅŸlatmak iÃ§in
                ...
                // Kare iÅŸleme gÃ¶revini baÅŸlat
                ...
                _logger.LogInformation("GerÃ§ek zamanlÄ± video OCR hizmeti baÅŸarÄ±yla baÅŸlatÄ±ldÄ±");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError("GerÃ§ek zamanlÄ± video OCR hizmeti baÅŸlatÄ±lamadÄ±", ex);
                OnOcrError("GerÃ§ek zamanlÄ± video OCR hizmeti baÅŸlatÄ±lamadÄ±", ex);
                return false;
            }
        }
        ...
        public async Task StopAsync()
        {
            try
            {
                ...
                _logger.LogInformation("GerÃ§ek zamanlÄ± video OCR hizmeti durduruluyor...");
                ...
                // Video yakalamayÄ± durdur
                await _videoCaptureService.StopCaptureAsync();

                _cancellationTokenSource?.Dispose();
                _cancellationTokenSource = null;

                _logger.LogInformation("GerÃ§ek zamanlÄ± video OCR hizmeti durduruldu");
            }
            catch (Exception ex)
            {
                _logger.LogError("GerÃ§ek zamanlÄ± video OCR hizmeti durdurulurken hata oluÅŸtu", ex);
                OnOcrError("GerÃ§ek zamanlÄ± video OCR hizmeti durdurulurken hata oluÅŸtu", ex);
            }
        }
        ...
        public async Task<VideoOcrResult> ProcessFrameAsync(Bitmap frame)
        {
            ...
            try
            {
                ...
                if (EnableComparison)
                {
                    // En iyi sonucu almak iÃ§in karÅŸÄ±laÅŸtÄ±rma yap
                ...
                {
                    // Tek motor kullan - Her seferinde yeni yaratmak yerine ServiceContainer'daki singleton servisi kullan
                    // Not: Bu sınıfa IOcrService inject edilebilir. 
                    // Ancak mevcut yapıya en az zarar verecek şekilde using ekliyoruz.
                    using (var ocrService = new OcrService(_logger, _appSettings))
                    {
                ...
                // Referans metin mevcutsa doÄŸruluk puanÄ±nÄ± hesapla
                ...
                // Son sonuÃ§lar kuyruÄŸa ekle
                _recentResults.Enqueue(result);
                while (_recentResults.Count > 10) // YalnÄ±zca son 10 sonucu sakla (Bellek sÄ±zÄ±ntÄ±larÄ±nÄ± Ã¶nlemek iÃ§in 100'den dÃ¼ÅŸÃ¼rÃ¼ldÃ¼)
                {
                    if (_recentResults.TryDequeue(out var oldResult))
                    {
                        oldResult.SourceFrame?.Dispose(); // Ã‡Ä±karÄ±lan eski bitmapleri RAM'den siliyoruz.
                    }
                }
            ...
            {
                stopwatch.Stop();
                _logger.LogError($"Kare iÅŸleminde hata oluÅŸtu {frameNum}", ex);
                OnOcrError($"Kare iÅŸleminde hata oluÅŸtu {frameNum}", ex, frameNum);
                return null;
            }
        }
        ...
        public void SetGroundTruth(string groundTruth)
        {
            ...
            _logger.LogInformation($"Referans metin ayarlandÄ±: {(_groundTruth.Length > 50 ? _groundTruth.Substring(0, 50) + "..." : _groundTruth)}");
        }
        ...
        private async Task ProcessFramesAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    // Bu metot, kareler yakalandÄ±ÄŸÄ±nda Ã§aÄŸrÄ±lÄ±r
            ...
            {
                _logger.LogError("Video OCR iÅŸleme dÃ¶ngÃ¼sÃ¼nde hata oluÅŸtu", ex);
                OnOcrError("Video OCR iÅŸleme dÃ¶ngÃ¼sÃ¼nde hata oluÅŸtu", ex);
            }
        }
        ...
        private async void OnFrameCaptured(object sender, FrameCapturedEventArgs e)
        {
            ...
            {
                _logger.LogError($"Yakalanan kare iÅŸleminde hata oluÅŸtu {e.FrameNumber}", ex);
                OnOcrError($"Yakalanan kare iÅŸleminde hata oluÅŸtu {e.FrameNumber}", ex, e.FrameNumber);
            }
        }

        private void OnVideoError(object sender, VideoErrorEventArgs e)
        {
            _logger.LogError($"Video hatasÄ±: {e.ErrorMessage}", e.Exception);
            OnOcrError($"Video hatasÄ±: {e.ErrorMessage}", e.Exception);
        }
