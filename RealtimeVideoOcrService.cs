    ...
    public class RealtimeVideoOcrService : IRealtimeVideoOcrService, IDisposable
    {
        ...
        public async Task<bool> StartAsync(int deviceIndex = 0)
        {
            try
            {
                if (IsRunning)
                {
                    _logger.LogWarning("GerÃ§ek zamanlÄ± video OCR zaten Ã§alÄ±ÅŸÄ±yor");
                    return true;
                }

                _logger.LogInformation("GerÃ§ek zamanlÄ± video OCR hizmeti baÅŸlatÄ±lÄ±yor...");

                // Video yakalamayÄ± baÅŸlatmak iÃ§in
                var captureStarted = await _videoCaptureService.StartCaptureAsync(deviceIndex);
                if (!captureStarted)
                {
                    _logger.LogError("Video yakalama baÅŸlatÄ±lamadÄ±");
                    return false;
                }
        ...
        public async Task<VideoOcrResult> ProcessFrameAsync(Bitmap frame)
        {
            ...
            try
            {
                ...
                {
                    // Tek motor kullan (mevcut uygulama ayarÄ±)
